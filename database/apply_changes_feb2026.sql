...
-- Based on: HR SLA Guide + Employee SQL Import
-- 
-- Changes:
-- 1. SLA policy response times → all set to 24 hours
-- 2. "Payslip Disputes" → renamed "Payslip Dispute" + priority HIGH
-- 3. Deactivate "Leave Assistance" (not in current SLA guide)
...
--   MEDIUM → Response: 24h  | Resolution: 48-72h (use 72h = 4320 min)
--   LOW    → Response: 24h  | Resolution: 56-120h (use 120h = 7200 min)
-- ============================================================

UPDATE sla_policies 
SET response_time = 1440,   -- 24 hours
    resolution_time = 1440  -- 24 hours
WHERE priority = 'high' AND is_active = 1;

UPDATE sla_policies 
SET response_time = 1440,   -- 24 hours
    resolution_time = 4320  -- 72 hours (3 days)
WHERE priority = 'medium' AND is_active = 1;

UPDATE sla_policies 
SET response_time = 1440,   -- 24 hours
    resolution_time = 7200  -- 120 hours (5 days)
WHERE priority = 'low' AND is_active = 1;

-- ============================================================
-- 2. RENAME "Payslip Disputes" → "Payslip Dispute" + priority HIGH
-- ============================================================

SET @dept_hr = (SELECT id FROM departments WHERE code = 'HR');
SET @cat_salary = (
    SELECT id FROM categories 
    WHERE name = 'Salary Dispute' AND department_id = @dept_hr 
    LIMIT 1
);

-- Rename "Payslip Disputes" to "Payslip Dispute"
UPDATE categories 
...
-- Set "Payslip Dispute" priority to HIGH
INSERT INTO category_priority_map (category_id, default_priority)
SELECT id, 'high' 
FROM categories 
WHERE name = 'Payslip Dispute' AND parent_id = @cat_salary 
LIMIT 1
ON DUPLICATE KEY UPDATE default_priority = 'high';

-- Also ensure "Draft Payslip" is mapped HIGH
INSERT INTO category_priority_map (category_id, default_priority)
SELECT id, 'high' 
FROM categories 
WHERE name = 'Draft Payslip' AND parent_id = @cat_salary 
LIMIT 1
ON DUPLICATE KEY UPDATE default_priority = 'high';
...
SET @cat_leave = (
    SELECT id FROM categories 
    WHERE name = 'Leave concerns' AND department_id = @dept_hr 
    LIMIT 1
);

UPDATE categories 
SET is_active = 0 
WHERE name = 'Leave Assistance' AND parent_id = @cat_leave;

-- ============================================================
-- 4. EMPLOYEE UPSERT FROM HARLEY DATABASE EXPORT
-- Source: employees (2).sql (u816220874_calendartype.employees)
-- Strategy: INSERT ... ON DUPLICATE KEY UPDATE
-- ============================================================

INSERT INTO employees (
    id, fname, lname, email, personal_email, contact, position, status, 
    Emp_Type, created_at, username, password, company, profile_image, 
    profile_picture, official_sched, role, admin_rights_hdesk
) VALUES
...
ON DUPLICATE KEY UPDATE
    ...
    -- Do NOT overwrite password if already set in production (comment out next line if needed)
    password      = VALUES(password),
    company       = VALUES(company),
    profile_picture = COALESCE(VALUES(profile_picture), profile_picture),
    official_sched = VALUES(official_sched),
    role          = VALUES(role),
    -- Preserve existing admin_rights_hdesk (never overwrite with NULL from Harley)
    admin_rights_hdesk = COALESCE(admin_rights_hdesk, VALUES(admin_rights_hdesk));

-- ============================================================
-- VERIFICATION
-- ============================================================

SELECT 'SLA Policies after update:' AS info;
SELECT priority, response_time, resolution_time FROM sla_policies WHERE is_active = 1 ORDER BY FIELD(priority, 'high', 'medium', 'low');
