...
class FirebaseNotificationSender {
    ...
    public function sendNewReplyNotification($ticketId, $fromUserId, $fromUserType, $message) {
        try {
            ...
            $stmt = $db->prepare("
                SELECT t.*, e.fname, e.lname, e.user_id as employee_id,
                       its.name as staff_name, its.user_id as staff_id
                FROM tickets t
                LEFT JOIN employees e ON t.employee_id = e.user_id
                LEFT JOIN it_staff its ON t.assigned_to = its.user_id
                WHERE t.ticket_id = ?
            ");
            ...
            if ($fromUserType === 'employee') {
                ...
                $recipientId = $ticket['staff_id'] ?? null;
                ...
                $fromName = $ticket['staff_name'] ?? 'IT Support';
            }
    ...
    public function sendStatusChangeNotification($ticketId, $newStatus, $changedBy) {
        try {
            ...
            return $this->sendToUser($ticket['employee_id'], 'employee', $notification);
