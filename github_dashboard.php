...
try {
    ...
    $ticketsQuery = "
        SELECT 
            ...
            (SELECT COUNT(*) FROM ticket_responses tr WHERE tr.ticket_id = t.ticket_id AND tr.is_internal = FALSE) as response_count
        FROM tickets t 
        LEFT JOIN employees e ON t.employee_id = e.id 
        LEFT JOIN it_staff s ON t.assigned_to = s.staff_id 
        $whereClause
...
<body class="bg-gray-50 min-h-screen">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center space-x-4 text-sm text-gray-600">
                                    ...
                                    <span class="flex items-center">
                                        ...
                                        <?php echo escape($ticket['employee_name']); ?>
                                    </span>
                                    <?php endif; ?>
                                    ...
                                    <span class="flex items-center">
                                        ...
                                        <?php echo escape($ticket['assigned_name']); ?>
                                    </span>
                                    <?php endif; ?>
    ...
    <div class="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4" id="mobileSearch" style="display: none;">
        <form method="GET" class="flex">
            <input type="text" 
                   name="search" 
                   value="<?php echo escape($filters['search']); ?>"
                   placeholder="Search tickets..." 
                   class="flex-1 px-3 py-2 border border-gray-300 rounded-l-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white border border-blue-600 rounded-r-md">
                <i class="fas fa-search"></i>
            </button>
        </form>
    </div>
    ...
    <script>
        ...
        // Auto-submit filters on change (already handled by onchange attributes)
    </script>
</body>
</html>